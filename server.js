...
const startService = async () => {
  try {
    ...
    adminTelegramService.setServerControl(serverControl);
    ...
    if (adminTelegramService.isAvailable()) {
      // Check if this was a restart requested by an admin
      const restartRequesterChatId = process.env.RESTART_REQUESTER_CHAT_ID;
      
      if (restartRequesterChatId) {
        // Send restart completion notification to the requesting admin
        adminTelegramService.bot.sendMessage(restartRequesterChatId,
          `✅ *SERVER RESTART COMPLETED*\n\n` +
          `The Attendance Automation System has been successfully restarted.\n\n` +
          `• All services are online\n` +
          `• Database connection established\n` +
          `• Scheduler resumed operations\n` +
          `• Bot services active\n\n` +
          `System is ready for normal operations.`,
          { parse_mode: 'Markdown' }
        ).catch(err => console.error('Error sending restart notification:', err));
        
        // Clear the restart requester env var
        delete process.env.RESTART_REQUESTER_CHAT_ID;
        
        console.log(`✅ Restart completion notification sent to admin: ${restartRequesterChatId}`);
      } else {
...
// Global server control functions
const serverControl = {
  /**
  ...
  shutdown: async () => {
    ...
    if (adminTelegramService && adminTelegramService.isAvailable()) {
      try {
        await adminTelegramService.sendSystemNotification(
          'System Shutdown',
          'The Attendance Automation System is shutting down gracefully.'
        );
      } catch (err) {
        console.error('Error sending shutdown notification:', err);
      }
   ...
   */
  restart: async (requesterChatId) => {
    console.log('Graceful restart requested...');
  ...
  getStatus: () => {
    ...
    const schedulerStatus = scheduler ? scheduler.getStatus() : { running: false };
    ...
    return {
      ...
      databaseConnected: !!process.env.MONGODB_URI,
      adminBotActive: adminTelegramService ? adminTelegramService.isAvailable() : false,
      userBotActive: telegramService ? telegramService.isAvailable() : false
    };
  }
};

// Export initialized services and control functions
module.exports = {
