...
class AdminTelegramService {
  ...
  /**
   * Set the server control instance
   */
...
_Attendance Automation System_`;
      ...
      const keyboard = {
        inline_keyboard: [
          ...
          [
            ...
            { text: '🖥️ Server Status', callback_data: 'admin_server_status' }
          ],
          [
    ...
    this.bot.onText(/\/serverstatus/, async (msg) => {
      const chatId = msg.chat.id;
      ...
      await this.handleServerStatusCommand(chatId);
    });

    // Handle /serverstop command
    this.bot.onText(/\/serverstop/, async (msg) => {
      const chatId = msg.chat.id;
      ...
      await this.handleServerStopCommand(chatId);
    });

    // Handle /serverrestart command
    this.bot.onText(/\/serverrestart/, async (msg) => {
      const chatId = msg.chat.id;
      ...
      await this.handleServerRestartCommand(chatId);
    });
      ...
      try {
        ...
        switch (data) {
          ...
          default:
            ...
            if (data.startsWith('admin_student_')) {
            ...
            } else if (data.startsWith('admin_server_')) {
              const action = data.replace('admin_server_', '');
              ...
              if (action === 'status') {
                ...
                await this.handleServerRestartCommand(chatId);
              } else if (action === 'confirm_stop') {
                await this.handleConfirmServerStopCommand(chatId);
              } else if (action === 'confirm_restart') {
                await this.handleConfirmServerRestartCommand(chatId);
              }
            }
            break;
        }
...
*🖥️ Server Control:*
• \`/serverstop\` - Stop the server gracefully
• \`/serverrestart\` - Restart the server
...
⚠️ *Warning:* Server control commands will affect the entire system. Use with caution.
...
_Admin Report by Attendance Automation System_`;
    ...
    try {
      ...
      const keyboard = {
        inline_keyboard: [
          ...
          [
            { text: '⏹️ Stop Server', callback_data: 'admin_server_stop' },
            { text: '🔄 Restart Server', callback_data: 'admin_server_restart' }
          ],
          [
   ...
   */
  async handleServerStatusCommand(chatId) {
    try {
      await this.bot.sendMessage(chatId, '⏳ *Checking Server Status...*', { parse_mode: 'Markdown' });

      // Get server status from server control if available
      let serverStatus = null;
      if (this.serverControl) {
        serverStatus = this.serverControl.getStatus();
      }

      // Get process information
      const uptime = serverStatus?.uptime || process.uptime();
      ...
      // Get memory usage
      const memUsage = serverStatus?.memoryUsage || process.memoryUsage();
      ...
      // Get database status
      const dbStatus = serverStatus?.databaseConnected ? 'Connected' : 
                      (process.env.MONGODB_URI ? 'Connected' : 'Not Connected (Mock Mode)');

      // Get heartbeat status
      const heartbeatStatus = schedulerStatus.lastHeartbeat ? 
        `Active (${new Date(schedulerStatus.lastHeartbeat).toLocaleString()})` : 
        'Not Available';

      const message = `🖥️ *Server Status Report*\n\n` +
        `━━━━━━━━━━━━━━━━━━━━━\n` +
        `*System Status:*\n` +
        `• Process: Running ✅\n` +
        `• Uptime: ${uptimeString}\n` +
        `• Memory Usage: ${memUsedMB} MB\n` +
        `• Environment: ${serverStatus?.environment || process.env.NODE_ENV || 'development'}\n\n` +
        
        `*Services Status:*\n` +
        `• Database: ${dbStatus}\n` +
        `• Scheduler: ${schedulerStatus.running ? 'Running ✅' : 'Stopped ❌'}\n` +
        `• Admin Bot: ${serverStatus?.adminBotActive ? 'Active ✅' : 'Active ✅'}\n` +
        `• User Bot: ${serverStatus?.userBotActive ? 'Active ✅' : (this.attendanceService ? 'Active ✅' : 'Inactive ❌')}\n\n` +
        
        `*Monitoring:*\n` +
        `• Heartbeat: ${heartbeatStatus}\n` +
        `• Last Attendance Check: ${schedulerStatus.lastRun ? new Date(schedulerStatus.lastRun).toLocaleString() : 'Never'}\n` +
        `• Next Scheduled Check: ${schedulerStatus.nextRun ? new Date(schedulerStatus.nextRun).toLocaleString() : 'Not Scheduled'}\n` +
        `━━━━━━━━━━━━━━━━━━━━━\n` +
        `_Status checked: ${new Date().toLocaleString()}_`;

      const keyboard = {
        inline_keyboard: [
          [
            { text: '🔄 Refresh Status', callback_data: 'admin_server_status' },
            { text: '📊 View Full Stats', callback_data: 'admin_stats' }
          ],
          [
            { text: '⏹️ Stop Server', callback_data: 'admin_server_stop' },
            { text: '🔄 Restart Server', callback_data: 'admin_server_restart' }
      ...
      await this.bot.sendMessage(chatId, 
        `❌ *Error*\n\nFailed to retrieve server status: ${error.message}`,
        { parse_mode: 'Markdown' }
      );
    }
  }
   ...
   */
  async handleServerStopCommand(chatId) {
    try {
      const warningMessage = `⚠️ *SERVER SHUTDOWN WARNING*\n\n` +
        `You are about to shut down the entire Attendance Automation Server.\n\n` +
        `**This will:**\n` +
        `• Stop all scheduled attendance checks\n` +
        `• Disconnect the database\n` +
        `• Stop both admin and user bots\n` +
        `• Require manual restart to resume service\n\n` +
        `**Are you sure you want to proceed?**\n\n` +
        `⚠️ *This action cannot be undone remotely!*`;

      const keyboard = {
        inline_keyboard: [
          [
            { text: '⛔ CONFIRM SHUTDOWN', callback_data: 'admin_server_confirm_stop' },
            { text: '❌ Cancel', callback_data: 'admin_server_status' }
          ]
        ]
      };
      ...
      console.error('Error handling server stop command:', error);
      await this.bot.sendMessage(chatId, 
        `❌ *Error*\n\nFailed to process server stop command: ${error.message}`,
        { parse_mode: 'Markdown' }
      );
    }
  }
  ...
  /**
   * Handle confirmed server stop command
   */
  async handleConfirmServerStopCommand(chatId) {
    try {
      // Check if server control is available
      if (!this.serverControl) {
        await this.bot.sendMessage(chatId, 
          '❌ *Service Unavailable*\n\nServer control service is not available.',
          { parse_mode: 'Markdown' }
        );
        return;
      }
      ...
      await this.bot.sendMessage(chatId, 
        ...
        `The server is now shutting down gracefully...\n\n` +
        `This bot will become unresponsive shortly.\n` +
        `Manual intervention will be required to restart the server.`,
        { parse_mode: 'Markdown' }
      );

      // Log the shutdown request
      console.log(`⚠️ Server shutdown initiated by admin: ${chatId}`);

      // Give the message time to send before shutdown
      setTimeout(async () => {
        try {
          await this.serverControl.shutdown();
        } catch (error) {
      ...
      console.error('Error confirming server stop:', error);
      await this.bot.sendMessage(chatId, 
        `❌ *Error*\n\nFailed to initiate server shutdown: ${error.message}`,
        { parse_mode: 'Markdown' }
      );
    }
  }
   ...
   */
  async handleServerRestartCommand(chatId) {
    try {
      const warningMessage = `🔄 *SERVER RESTART WARNING*\n\n` +
        `You are about to restart the entire Attendance Automation Server.\n\n` +
        `**This will:**\n` +
        ...
        `⚠️ *The bot will be temporarily unresponsive during restart*\n\n` +
        `**Are you sure you want to proceed?**`;

      const keyboard = {
        inline_keyboard: [
          [
            { text: '🔄 CONFIRM RESTART', callback_data: 'admin_server_confirm_restart' },
            { text: '❌ Cancel', callback_data: 'admin_server_status' }
          ]
        ]
      };
      ...
      console.error('Error handling server restart command:', error);
      await this.bot.sendMessage(chatId, 
        `❌ *Error*\n\nFailed to process server restart command: ${error.message}`,
        { parse_mode: 'Markdown' }
      );
    }
  }
  ...
  /**
   * Handle confirmed server restart command
   */
  async handleConfirmServerRestartCommand(chatId) {
    try {
      // Check if server control is available
      if (!this.serverControl) {
        await this.bot.sendMessage(chatId, 
      ...
      await this.bot.sendMessage(chatId, 
        ...
        `Please wait for the restart completion notification.`,
        { parse_mode: 'Markdown' }
      );

      // Log the restart request
      console.log(`🔄 Server restart initiated by admin: ${chatId}`);

      // Give the message time to send before restart
      setTimeout(async () => {
        try {
          await this.serverControl.restart(chatId);
        } catch (error) {
      ...
      console.error('Error confirming server restart:', error);
      await this.bot.sendMessage(chatId, 
        `❌ *Error*\n\nFailed to initiate server restart: ${error.message}`,
        { parse_mode: 'Markdown' }
      );
    }
  }
